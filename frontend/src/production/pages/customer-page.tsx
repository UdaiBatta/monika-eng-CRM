  ...
  return (
    <div className="mx-auto flex max-w-[1580px] flex-col gap-5">
      ...
      <Tabs defaultValue="overview">
        ...
        {hasPermission(user, "projects.project.view") ? <TabsContent value="projects" className="mt-4"><Card><CardHeader className="flex-row items-start justify-between"><div><CardTitle>Customer projects</CardTitle><CardDescription>Released orders that require an operational Project and Engineering handoff.</CardDescription></div><Link to="/app/projects"><Button size="sm" variant="outline">Open register</Button></Link></CardHeader><CardContent>{projects.isPending ? <ERPLoadingState rows={4} /> : !projects.data?.results.length ? <ERPEmptyState title="No Projects yet" description="Project work appears after a project-required Sales Order is released." /> : <div className="divide-y">{projects.data.results.map((project) => <Link key={project.id} to={`/app/projects/${project.id}`} className="flex items-center gap-3 py-3 first:pt-0 hover:text-primary"><Building2 /><div className="min-w-0 flex-1"><p className="font-medium">{project.project_number} · {project.project_name}</p><p className="text-xs text-muted-foreground">Engineering: {project.engineering_owner_name || "Unassigned"} · {project.next_action}</p></div><ERPStatusBadge value={project.status} /></Link>)}</div>}</CardContent></Card></TabsContent> : null}
